...
const TableElem = ({ columns, data, ui, onChange, isDisable }) => {
        ...
        <tbody>
          {rows?.map((row, index) => (
            ...
            >
              ...
              <td key={index * 4 + 4} width="50%">{`${
                data[`${row}`]['description']
              }`}</td>
              <td key={index * 4 + 1} width="20%">
                {`${data[`${row}`]['label']}`}{' '}
              </td>
              <td key={index * 4 + 2} width="20%">
                {typeof data[`${row}`]['value'] === 'number' ? (
                  <NumericFormat
                    style={{
                      border: 0,
                      backgroundColor: isDisable === true ? 'white' : 'white',
                      width: '80%',
                      color: `${
                        data[`${row}`]['color']
                          ? data[`${row}`]['color']
                    ...
                    // value={
                    // 	(data[`${row}`]['name'] === 'nb' || data[`${row}`]['name'] === 'h' || data[`${row}`]['name'] === 'b' || data[`${row}`]['name'] === 'wLimQ' || data[`${row}`]['name'] === 'wLimTotal')
